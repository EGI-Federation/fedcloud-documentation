...
* installing docker on top of an existing VM (by following `docker installation
  instructions <https://docs.docker.com/engine/installation/>`_; or
...
You can run several docker clusters management tools on the EGI Cloud, each
tool has its own specifics, but there are plenty of helpers to aid in their
setup. Here we cover how to configure Kubernetes by using `ansible <https://www.ansible.com/>`_.
